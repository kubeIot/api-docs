...
func ConcatJSON(blobs ...[]byte) []byte {
	...
	for i, b := range blobs {
		...
		if len(b) < 3 {
			// yep empty but also the last one, so closing this thing
			if i == last && a > 0 {
		...
		idx = 0
		if a > 0 {
			// we need to join with a comma for everything beyond the first non-empty item
		...
		if i != last {
			// not the last one, strip brackets
			if _, err := buf.Write(b[idx : len(b)-1]); err != nil {
				log.Println(err)
			}
		} else {
			// last one, strip only the leading bracket
...
func buildnameIndex(tpe reflect.Type, idx, reverseIdx map[string]string) {
	for i := 0; i < tpe.NumField(); i++ {
		...
		if targetDes.Anonymous {
			// walk embedded structures tree down first
		...
		if tag := targetDes.Tag.Get("json"); tag != "" {
			...
			if nm == "" {
				// empty string means we want to use the Go name
				nm = targetDes.Name
			}
